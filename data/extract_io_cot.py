"""
Extract IO Chain-of-Thought - Step 6a of ExecVerify Data Pipeline

Extracts input-output prediction chain-of-thought reasoning by having
a model predict outputs given inputs and function code.

Input: filtered_all_dataset_with_difficulties.json
Output: filtered_all_dataset_with_difficulties_subset_with_io_cot.json,
        io_sft_dataset.json
"""
...
def get_io_prompt(func_str: str, func_name: str, exec_result: str, args: str = "????") -> str:
    """
    Build prompt for input-output prediction.
    
    Args:
        func_str: Function definition code
        func_name: Function name
        exec_result: Result placeholder or actual result
...
def generate_prompt(tokenizer, func_name: str, func_str: str, 
    ...
    """Generate chat prompt for IO prediction."""
    return build_chat_prompt(
        tokenizer,
        "You are a programming expert",
        get_io_prompt(func_str, func_name, exec_result, args_str)
    )
...
def format_for_sft(sample: dict, cot_key: str, prompt_func) -> dict:
    ...
    question = prompt_func(
        sample['func_str'],
        sample['func_name'],
        "????" if cot_key == 'io_cot' else sample['result'],
        sample['func_args'] if cot_key == 'io_cot' else "????"
    )
...
def main():
    ...
    logger.info("Starting IO chain-of-thought extraction...")
    ...
    # Take subset
    subset_size = config['dataset_limits']['io_cot_subset']
    logger.info(f"Using first {subset_size} samples")
    filtered_subset = filtered_all_dataset[:subset_size]
    ...
    # Generate prompts
    logger.info("Generating IO prediction prompts...")
    chat_prompts = []
    ...
    for sample, sample_outputs in zip(filtered_subset, generated_outputs):
        ...
        try:
            for output in sample_outputs.outputs:
                ...
                if ast.literal_eval(gt_exec_output) == ast.literal_eval(predict_output):
                    sample['io_cot'] = output_text
                    dataset_with_io_cot.append(sample)
                    break
        except Exception:
            pass
    
    logger.info(f"Successfully extracted {len(dataset_with_io_cot)} samples with IO CoT")
    
    # Save dataset with CoT
    output_file = config['output_files']['io_cot_dataset']
    save_json(dataset_with_io_cot, output_file, logger)
    ...
    for sample in dataset_with_io_cot[:sft_size]:
        sft_sample = format_for_sft(sample, 'io_cot', get_io_prompt)
        io_sft_dataset.append(sft_sample)
    
    # Save SFT dataset
    sft_output_file = config['output_files']['io_sft_dataset']
    save_json(io_sft_dataset, sft_output_file, logger)
    
    logger.info("IO chain-of-thought extraction complete!")
